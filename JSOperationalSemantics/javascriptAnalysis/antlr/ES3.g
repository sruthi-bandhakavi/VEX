...
// Imaginary
	...
	PINC ;
	POS ;
	FUNBODY ; // Added by Sruthi
	ROOT_ID; // Added by Sruthi
	// IFELSE ; //Added by Sruthi
}

@lexer::header{package javascriptAnalysis.parser; }
...
fragment EscapeSequence returns [String es]
	:
	BSLASH 
	(
...
StringLiteral 
	: SQUOTE st+=( ~( SQUOTE | BSLASH | LineTerminator ) | EscapeSequence)* SQUOTE 
	| DQUOTE st+=( ~( DQUOTE | BSLASH | LineTerminator ) | EscapeSequence)* DQUOTE 
	;
...
nameValuePair
	: propertyName COLON assignmentExpression -> ^( NAMEDVALUE propertyName assignmentExpression )
	| getterOrSetterExpression
	;
	
getterOrSetterExpression
	: getorset=Identifier name=Identifier formalParameterList functionBody
	->  ^( FUNCTION $name formalParameterList functionBody )
	;
	
/*	{isGetOrSet($getorset.text)}?
		//-> $getterOrSetterExpression
*/
...
leftHandSideExpression
	...
	(
		...
		| LBRACK ex=expression RBRACK  -> ^( BYINDEX $leftHandSideExpression expression ) 
...
statementTail
	...
	| expressionStatement // : ASSIGN | MULASS | DIVASS | MODASS | ADDASS | SUBASS | SHLASS | SHRASS | SHUASS | ANDASS | XORASS | ORASS | CEXPR
	| ifStatement // IF
	...
	| withStatement // WITH
	| labelledStatement // LABELLED
	| switchStatement // SWITCH
	| throwStatement // THROW
	| tryStatement // TRY, CATCH, DEFAULT
	;
...
variableStatement
	: VAR variableDeclaration ( COMMA variableDeclaration )* semic -> ^( VAR variableDeclaration+ )
	| CONST variableDeclaration ( COMMA variableDeclaration )* semic -> ^( VAR variableDeclaration+ )
	;
...
/* The predicate is there just to get rid of the warning. ANTLR will handle the dangling else just fine.*/
    /*: IF LPAREN expression RPAREN ifStat=statement 
      ({ input.LA(1) == ELSE }? ELSE elseStat=statement                    ->  ^(IF expression $ifStat $elseStat)
     ...
     @init{$ifStatement::elsevar = false;}
     @after{$ifStatement::elsevar = false;}
//The predicate is there just to get rid of the warning. ANTLR will handle the dangling else just fine.
	: IF LPAREN expression RPAREN ifstat=statement ( { input.LA(1) == ELSE }? {$ifStatement::elsevar = true;} ELSE elsestat=statement)? 
	         -> {$ifStatement::elsevar == true}? ^( IF expression $ifstat $elsestat)
	         -> ^( IF expression $ifstat )
...
*/
forStatement
	: FOR^ (EACH!)? LPAREN! forControl RPAREN! statement
	;
...
no line terminators allowed between CONTINUE and the optional identifier.
As an optimization we check the la first to decide whether there is an identifier following.
*/
continueStatement
...
no line terminators allowed between BREAK and the optional identifier.
As an optimization we check the la first to decide whether there is an identifier following.
*/
breakStatement
...
functionBody
	...
	-> ^( FUNBODY[$lb, "FUNBODY"] sourceElement* )
	;

// $>
	
// $<	Program (14)
// Sruthi: Added the imaginary element ROOT_ID to anchor the root
program
	: se = sourceElement*
	-> ^(ROOT_ID sourceElement*)
	;
