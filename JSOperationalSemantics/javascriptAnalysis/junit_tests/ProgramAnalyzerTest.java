...
import javascriptAnalysis.parser.ProgramAnalyzer;
...
public class ProgramAnalyzerTest extends TestCase {
	ES3Lexer lex;
	
	public ProgramAnalyzerTest(String name) {
		super(name);
	}
	...
	private State walkProgram(String filename) throws IOException,
		...
		ProgramAnalyzer vt = new ProgramAnalyzer(filename);
	...
	private StateSet walkProgram(String fn, String str1)
		...
		// Create the tree walker 
		ProgramAnalyzer vt = new ProgramAnalyzer(fn);
		...
		State post_preWalk = initsts.next();
		// Process the program now
		State resultState = vt.walk(root,post_preWalk, null);
	...
	public static TestSuite suite(){
		TestSuite suite = new TestSuite(ProgramAnalyzerTest.class);
		return suite;
	}
